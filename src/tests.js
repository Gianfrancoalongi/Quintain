...
test("new system has only input to stdout edge", function() {
    ...
    deepEqual( system.edges.get_edge(0), {id:0, from:'stdin', to:'device_1'});
});

test("adding branch to empty system will put it between stdin and device", function() {
    var system = new_system();
    var branch = new_branch();
    system.add_node_between(branch.id);
    deepEqual( system.edges.get_edge(0), {id:0, from:'stdin', to:'branch_1'});
    deepEqual( system.edges.get_edge(1), {id:1, from:'branch_1', to:'device_1'});
});

test("adding replicator to empty system will put it between stdin and device", function() {
    var system = new_system();
    var replicator = new_replicator();
    system.add_node_between(replicator.id);
    deepEqual( system.edges.get_edge(0), {id:0,from:'stdin',to:'replicator_1'});
    deepEqual( system.edges.get_edge(1), {id:1,from:'replicator_1',to:'device_1'});
});

test("adding splitter to empty system will put it between stdin and device", function() {
    var system = new_system();
    var splitter = new_splitter();
    system.add_node_between(splitter.id);
    deepEqual( system.edges.get_edge(0), {id:0,from:'stdin',to:'splitter_1'});
    deepEqual( system.edges.get_edge(1), {id:1,from:'splitter_1',to:'device_1'});
});

test("adding function to empty system will put it between stdin and device", function() {
    var system = new_system();
    var funct = new_function();
    system.add_node_between(funct.id);
    deepEqual( system.edges.get_edge(0), {id:0,from:'stdin',to:'function_1'});
    deepEqual( system.edges.get_edge(1), {id:1,from:'function_1',to:'device_1'});
});
